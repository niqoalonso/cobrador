                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <p class="card-title-desc">Seleccione un cliente al cual desea ingresar nueva postura de pago.</p>
                        </div>
                        <div class="row divBotonRendir" @if(count($posturas) == 0) style="display: none;" @endif>
                            <div class="col-12 mt-2">
                                <div class="d-grid gap-2">
                                    <button class="btn btn-primary" type="button" onclick="RendirPosturas()"><i class="fa fa-check-circle"></i> Rendir Ahora</button>
                                  </div>
                            </div>
                        </div>
                        <div class="card-body divTablaDatos" @if(count($posturas) == 0) style="display: none;" @endif>
                            <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                                ...
                                <tbody>
                                    @foreach ($posturas as $item)
                                        <tr>
                                            <td>{{$item->Arriendo->Empresa->alias}}</td>
                                            <td>{{$item->sku}}</td>
                                            <td>${{$item->total}}</td>
                                            <td>{{$item->TipoPago->nombre}}</td>
                                            <td>{{$item->fecha_emision}}</td>
                                            <td class="text-center">
                                                <button class="btn btn-sm btn-info" onclick="verDetallePostura(this.id)" id="{{$item->sku}}"><i class="fa fa-eye"></i></button>
                                            </td>
                                        </tr>    
                                    @endforeach
                                </tbody>
                        ...
                        <div class="row divNoRigistros" @if(count($posturas) != 0) style="display: none;" @endif>
                            <div class="col-lg-12">
                                <div class="card-header ">
                                    <h5 class="card-title text-center">No se han encontrado posturas ingresador.</h5>
                                </div>
                            </div>
                        </div>
...
@section('js')
    ...
    <script src="{{asset('pages/js/RendicionPostura.js')}}"></script>
